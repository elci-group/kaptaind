use crate::diff::lang::adapter::Language;
use crate::diff::version::cache::VersionCache;
...
pub fn detect_all(cache: &mut VersionCache, repo_path: &Path) -> HashMap<Language, LanguageVersion> {
    let mut map = HashMap::new();

    let detectors: &[(&str, Language, fn(&Path) -> Option<LanguageVersion>)] = &[
        ("rust",       Language::Rust,       detect_rust),
        ("go",         Language::Go,         detect_go),
        ("python",     Language::Python,     detect_python),
        ("typescript", Language::TypeScript, detect_typescript),
        ("javascript", Language::JavaScript, detect_javascript),
        ("kotlin",     Language::Kotlin,     detect_kotlin),
        ("swift",      Language::Swift,      detect_swift),
        ("vue",        Language::Vue,        detect_vue),
        ("svelte",     Language::Svelte,     detect_svelte),
        ("astro",      Language::Astro,      detect_astro),
        ("scss",       Language::Scss,       |_| None),
        ("htmlcss",    Language::HtmlCss,    |_| None),
    ];

    for &(lang_key, lang, detector_fn) in detectors {
        // Try cache first
        if let Some(cached) = cache.get(lang_key) {
            map.insert(lang, cached);
            continue;
        }
        // Run detector
...
fn detect_npm_dep_version(repo_path: &Path, package: &str) -> Option<LanguageVersion> {
    ...
    for section in &["dependencies", "devDependencies"] {
        if let Some(ver) = val.get(*section).and_then(|d| d.get(package)).and_then(|v| v.as_str()) {
            let clean = ver.trim_start_matches(|c: char| c == '^' || c == '~' || c == '>' || c == '=' || c == ' ');
            return Some(LanguageVersion {
