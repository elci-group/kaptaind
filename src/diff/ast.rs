...
fn api_score_inner(
    ...
    for event in &cluster.events {
        for path in &event.paths {
            ...
            // Generated/minified artifacts are not live API (rev 28).
            if is_generated_artifact(path, &resolved) {
                continue;
            }
...
/// Generated and minified artifacts are not live API: regeneration churn and
/// bundle rebuilds must not register as API-surface change or drive version
/// bumps. Detection is by filename convention and by standard codegen header
/// markers in the first 4 KiB (adapter-200 backlog item 6, rev 28).
fn is_generated_artifact(path: &Path, resolved: &Path) -> bool {
    let name = path
        .file_name()
        .unwrap_or_default()
        .to_string_lossy()
        .to_lowercase();
    // Minified bundles and common codegen filename conventions.
    ...
    // Standard codegen header markers (Go `Code generated`, .NET
    // `<auto-generated>`, GraphQL/Relay `@generated`).
    if let Ok(bytes) = std::fs::read(resolved) {
        let head = String::from_utf8_lossy(&bytes[..bytes.len().min(4096)]);
        for line in head.lines().take(5) {
            let l = line.trim();
            if l.contains("<auto-generated")
                || l.contains("@generated")
                || (l.contains("Code generated") && l.contains("DO NOT EDIT"))
            {
                return true;
            }
        }
...
"#,
    ...
    #[test]
    fn generated_artifacts_do_not_register_as_api() {
        let dir = tempdir().expect("temp dir");
        let files = [
            // Minified bundle: minified symbols are not live API.
            ("dist/bundle.min.js", "export function minifiedLeak() {}\n"),
            // Protobuf codegen.
            ("gen/api.pb.go", "func Generated() {\n}\n"),
            // Standard codegen marker in the header.
            (
                "src/client.ts",
                "// Code generated by protoc-gen-ts. DO NOT EDIT.\nexport function leaked() {}\n",
            ),
            // Hand-written control: must still register.
            ("src/real.ts", "export function realApi() {}\n"),
        ];
        ...
        let cluster = cluster_with_event(FsEvent {
            paths: files.iter().map(|(rel, _)| PathBuf::from(rel)).collect(),
            kind: FsEventKind::Create,
            timestamp: Utc::now(),
        });

        let analysis = api_score(&cluster, dir.path());
        // Only the hand-written control file may contribute surface.
        assert_eq!(
            analysis.signatures, 1,
            "generated/minified files must not register as live API"
        );
    }
