...
pub fn sanitize_orb_toml(content: &str, extra_fragments: &[String]) -> Result<SanitizeReport> {
    let mut doc: DocumentMut = content
        .parse()
        .context("failed to parse .orb content as TOML")?;
...
fn is_confirmed_private(repo_path: &Path) -> Option<bool> {
        ...
        .output()
    {
        Ok(output) => output,
        Err(error) => {
            tracing::debug!(error = %error, "gh unavailable; treating repo visibility as unknown");
            return None;
        }
    };
...
pub fn sanitize_staged_orb_files(
    ctx: &RepoContext,
    config: &OrbSanitizeConfig,
...
"#;
        ...
        assert!(report
            .sanitized_toml
            .contains(r#"group = "release-management""#));
        assert!(report
            .sanitized_toml
            .contains(r#"model = "openai/gpt-oss-120b""#));
        assert!(report.sanitized_toml.contains("installable = true"));
        assert!(!report
            .sanitized_toml
            .contains("gsk_FAKEKEYFAKEKEYFAKEKEYFAKEKEYFAKEKEYFA"));
    }
...
"#;
        let report = sanitize_orb_toml(toml, &[]).unwrap();
        assert!(
            report.redacted_keys.is_empty(),
            "unexpected redactions: {:?}",
...
"#;
        ...
        assert_eq!(
            with_extra.redacted_keys,
            vec!["tool.internal_hostname_hint".to_string()]
        );
    }
...
"#;
        ...
        assert!(report
            .redacted_keys
            .contains(&"endpoints[0].token".to_string()));
        assert!(report
            .redacted_keys
            .contains(&"endpoints[1].token".to_string()));
    ...
    fn github_owner_repo_parses_both_ssh_and_https_remotes() {
        ...
        assert_eq!(
            parse_github_owner_repo("https://gitlab.com/elci-group/kaptaind.git"),
            None
        );
    }
